...
# Каждый стиль содержит яркое описание сцены + нужную композицию/свет
Style = Dict[str, Any]
...
# Усилители тематики — добавляются в промпт (мелкие предметы/фактура, которые «делают сцену»)
THEME_BOOST = {
...
async def start_generation_for_preset(update: Update, context: ContextTypes.DEFAULT_TYPE, preset: str):
    ...
    # Сильная фиксация идентичности + кино-свет
    prompt_core = (
    ...
    try:
        # 3 разных семени — выбор лучшего «себя»
