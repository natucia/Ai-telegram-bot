# === Telegram LoRA Bot — CINEMA SHOTS v3 ===
# Flux LoRA trainer • pinned versions • кино-стили с атрибутами • must-include • anti-headshot
...
STYLE_PRESETS: Dict[str, Style] = {
    ...
    "Серфер": {
        "desc": "Брызги, блики, доска в руках и ломающееся море за спиной — летний спортфильм.",
...
SCENE_NEGATIVE = {
    ...
    # портретам тоже даём мягкие исключения, но без анти-головы
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "в **узнаваемых кино-сценах** — от пирата на палубе до космопилота.\n\n"
...
async def start_generation_for_preset(update: Update, context: ContextTypes.DEFAULT_TYPE, preset: str):
    ...
    try:
        # 4 разных семени — выше шанс «правильной сцены»
